...
struct tmag5273_config {
	...
	size_t t_start_power_up;         /** power up time after the VCC pin is set to high */
 ...
 */
static int tmag5273_check_manufacturer_id(const struct i2c_dt_spec *i2c)
{
...
#if CONFIG_PM_DEVICE
/**
 * @brief check if performing trigger related stuff is allowed in the current context
 *
 * @retval 0 if allowed
 * @retval -EBUSY if not allowed
 * @retval see @ref pm_device_state_get error codes
 */
static uint8_t tmag5273_pm_is_modifiable(const struct device *dev)
{
...
/**
 * @brief change the power mode of the sensor
 *
 * @param dev handle to the sensor
 * @param action target mode
 *
 * @retval 0 on success
 * @retval -ENOTSUP if mode not supported
 * @retval see @ref i2c_reg_update_byte_dt for further return codes
 */
static int tmag5273_pm_ctrl(const struct device *dev, enum pm_device_action action)
{
	...
	case PM_DEVICE_ACTION_SUSPEND:
		target_mode = TMAG5273_OPERATING_MODE_SLEEP;
		break;
	default:
...
static int tmag5273_sample_fetch(const struct device *dev, enum sensor_channel chan)
{
	...
	case SENSOR_CHAN_MAGN_XYZ:
		if (!drv_cfg->x_axis && !drv_cfg->y_axis && !drv_cfg->z_axis) {
				...
				struct sensor_value *val)
{
	...
	if (drv_cfg->x_axis && (all_mag_axis || (chan == SENSOR_CHAN_MAGN_X))) {
		tmag5273_channel_b_field_convert(drv_data->x_sample, drv_data->xyz_range,
						 val + val_offset);
		val_offset++;
	}

	if (drv_cfg->y_axis && (all_mag_axis || (chan == SENSOR_CHAN_MAGN_Y))) {
		tmag5273_channel_b_field_convert(drv_data->y_sample, drv_data->xyz_range,
						 val + val_offset);
		val_offset++;
	}

	if (drv_cfg->z_axis && (all_mag_axis || (chan == SENSOR_CHAN_MAGN_Z))) {
...
static inline int tmag5273_init_device_config(const struct device *dev)
{
	...
	drv_data->conversion_time_us =
		TMAG5273_T_CONVERSION_US((FIELD_GET(regdata, TMAG5273_CONV_AVB_MSK)),
					 (drv_cfg->x_axis + drv_cfg->y_axis + drv_cfg->z_axis));
...
static inline int tmag5273_init_sensor_settings(const struct tmag5273_config *drv_cfg)
{
	...
	if (drv_cfg->x_axis) {
		regdata |= TMAG5273_MAG_CH_EN_X;
	}
	if (drv_cfg->y_axis) {
		regdata |= TMAG5273_MAG_CH_EN_Y;
	}
	if (drv_cfg->z_axis) {
		regdata |= TMAG5273_MAG_CH_EN_Z;
	}
...
static int tmag5273_init(const struct device *dev)
{
	...
	if (gpio_is_ready_dt(&drv_cfg->supply_gpio)) {
		...
		k_usleep(drv_cfg->t_start_power_up);
	}

	if (drv_cfg->i2c_update && (tmag5273_check_manufacturer_id(&drv_cfg->i2c) < 0)) {
...
#if CONFIG_PM_DEVICE
	/* wakeup if the sensor was in sleep mode (non-power cycle reboot */
	if (!tmag5273_pm_wakeup(dev)) {
		LOG_ERR("device wakeup failed");
		return -EIO;
	}
#endif
	...
	retval = tmag5273_check_manufacturer_id(&drv_cfg->i2c);
	if (retval < 0) {
		return retval;
	}
...
#define TMAG5273_DEFINE(inst)                                                                      \
	...
	static const struct tmag5273_config tmag5273_driver_cfg##inst = {                          \
		...
		.x_axis = (bool)(DT_INST_PROP(inst, axis) & TMAG5273_DT_AXIS_X),                   \
		.y_axis = (bool)(DT_INST_PROP(inst, axis) & TMAG5273_DT_AXIS_Y),                   \
		.z_axis = (bool)(DT_INST_PROP(inst, axis) & TMAG5273_DT_AXIS_Z),                   \
		...
		.t_start_power_up = DT_INST_PROP(inst, t_start_power_up),                          \
		...
		IF_ENABLED(CONFIG_PM_DEVICE, (.pm_i2c_workaround = DT_INST_PROP(                   \
